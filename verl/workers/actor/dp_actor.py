...
class DataParallelPPOActor(BasePPOActor):
    def __init__(
        ...
        self.global_caption_loss = "global" in self.config.reward_type
    ...
    def update_policy(self, data: DataProto) -> dict[str, Any]:
        ...
        non_tensor_select_keys = ["multi_modal_inputs"]
        if self.global_caption_loss:
            select_keys.extend(["caption_mask", "global_caption_advantages"])
        ...
        for _ in range(self.config.ppo_epochs):
            ...
            for mini_batch in mini_batches:
                ...
                if self.global_caption_loss:
                    total_caption_tokens = torch.sum(mini_batch.batch["caption_mask"])
                    dist.all_reduce(total_caption_tokens, op=dist.ReduceOp.SUM)
                if self.config.dynamic_batching:
                ...
                for micro_batch in micro_batches:
                    ...
                    if self.global_caption_loss:
                        caption_mask = model_inputs["caption_mask"]
                        global_caption_advantages = model_inputs["global_caption_advantages"]
                        pg_caption_loss, pg_caption_metrics = compute_policy_loss(
                            old_log_probs=old_log_probs,
                            log_probs=log_probs,
                            advantages=global_caption_advantages,
                            response_mask=caption_mask,
                            clip_ratio_low=self.config.clip_ratio_low,
                            clip_ratio_high=self.config.clip_ratio_high,
                            clip_ratio_dual=self.config.clip_ratio_dual,
                            tau_positive=self.config.tau_positive,
                    ...
                    if self.global_caption_loss:
                        global_caption_loss = self.config.caption_loss_weight * pg_caption_loss * torch.sum(caption_mask) * self.world_size / total_caption_tokens
                        batch_metrics_cap = {f"actor_caption/{k}": v for k, v in pg_caption_metrics.items()}
                        batch_metrics_cap["actor/pg_caption_loss"] = pg_caption_loss.detach().item()
                        append_to_dict(metrics, batch_metrics_cap)
                        loss = (1 - self.config.caption_loss_weight) * loss + self.config.caption_loss_weight * global_caption_loss
