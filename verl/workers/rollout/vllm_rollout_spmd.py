...
class vLLMRollout(BaseRollout):
    def __init__(
        ...
        **kwargs,
        ...
        self.cap_bos, self.cap_eos = tokenizer.encode("<caption>\n"), tokenizer.encode("</caption>\n")
    ...
    def update_sampling_params(self, **kwargs):
        ...
        if kwargs:
            if 'eos_token_id' in kwargs:
                eos_val = kwargs.pop('eos_token_id')
                if eos_val is not None:
                    current_stop = getattr(self.sampling_params, 'stop_token_ids', None)
                    current_stop = list(current_stop) if current_stop else []
                    
                    if isinstance(eos_val, (list, tuple)):
                        for val in eos_val:
                            if val not in current_stop:
                                current_stop.append(val)
                    else:
                        if eos_val not in current_stop:
                            current_stop.append(eos_val)
                            
                    kwargs['stop_token_ids'] = current_stop
    ...
    def generate_sequences(self, prompts: DataProto) -> DataProto:
        ...
        caption_mask = VF.get_caption_mask(
            response_ids=response_ids, bos_token_ids=self.cap_bos, eos_token_ids=self.cap_eos, dtype=attention_mask.dtype
        )
