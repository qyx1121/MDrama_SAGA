            ...
            score = self.reward_fn(
                {
                    ...
                    "task_type": data.non_tensor_batch["task_type"][i],
                    "gt_graph": data.non_tensor_batch["gt_graph"][i],
                    "gt_entities": data.non_tensor_batch["gt_entities"][i] if "gt_entities" in data.non_tensor_batch else ""
                }
            )
...
class BatchFunctionRewardManagerMixin:
    ...
    def compute_reward_batch(self, data: DataProto) -> Tuple[torch.Tensor, dict[str, list[float]]]:
        ...
        scores = self.reward_fn(
            reward_inputs, 
            llm_judge_ip=self.config.llm_judge_ip, 
            llm_judge_port=self.config.llm_judge_port,
            graph_ip = self.config.graph_ip,
            graph_port = self.config.graph_port,
            embedding_ip = self.config.embedding_ip,
            embedding_port = self.config.embedding_port
            )
        reward_tensor = torch.zeros_like(data.batch["responses"], dtype=torch.float32)
        reward_metrics = defaultdict(list)
