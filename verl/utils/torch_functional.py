...
def get_caption_mask(
    response_ids: torch.Tensor, bos_token_ids: List[int] = [27, 23347, 397], eos_token_ids: List[int] = [522, 23347, 397], dtype: torch.dtype = torch.long
):
    """Get the mask for the response ids, the mask will be 0 after the first eos token.

    eos_token_id can be int or list: 1 or [1, 2].
    ```
    e.g. eos_token = [5, 1], bos_token = [2, 4]
    response_ids:  [0, 0, 2, 4, 3, 5, 1, 0, 0]
    response_mask: [0, 0, 0, 0, 1, 0, 0, 0, 0]
    ```
    """
    ...
    eos_len = len(eos_token_ids)

    bos_tensor = torch.tensor(bos_token_ids, device=device)
    eos_tensor = torch.tensor(eos_token_ids, device=device)

    bos_windows = response_ids.unfold(1, bos_len, 1)
    eos_windows = response_ids.unfold(1, eos_len, 1)

    bos_matches = torch.all(bos_windows == bos_tensor, dim=2)
    eos_matches = torch.all(eos_windows == eos_tensor, dim=2)

    bos_start_indices = torch.argmax(bos_matches.int(), dim=1)
    eos_start_indices = torch.argmax(eos_matches.int(), dim=1)

    # Check if a match was actually found for each sequence
    any_bos_match = torch.any(bos_matches, dim=1)
    any_eos_match = torch.any(eos_matches, dim=1)

    bos_pos = bos_start_indices + bos_len

    eos_pos = eos_start_indices

    bos_pos = torch.where(any_bos_match, bos_pos, seq_len + 1)
    eos_pos = torch.where(any_eos_match, eos_pos, seq_len)

    indices = torch.arange(seq_len, device=device).unsqueeze(0)

    bos_pos = bos_pos.unsqueeze(1)
    eos_pos = eos_pos.unsqueeze(1)

    mask = (indices >= bos_pos) & (indices < eos_pos)
