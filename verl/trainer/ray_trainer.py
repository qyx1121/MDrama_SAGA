...
def compute_advantage(data: DataProto, adv_estimator: AdvantageEstimator, gamma: float = 1.0, lam: float = 1.0):
    ...
    adv_inputs = {
        ...
        "global_caption_rewards": data.batch.get("global_caption_rewards", None),
        "response_mask": data.batch["response_mask"],
        "caption_mask": data.batch.get("caption_mask", None),
    ...
    if isinstance(advantages, tuple):
        data.batch["advantages"] = advantages[0]
        data.batch["returns"] = returns[0]

        data.batch["global_caption_advantages"] = advantages[1]
        data.batch["global_caption_returns"] = returns[1]
    else:
...
class RayPPOTrainer:
    ...
    def _validate(self) -> dict[str, Any]:
        ...
        for batch_dict in self.val_dataloader:
            ...
            test_gen_batch.meta_info["max_frames"] = self.config.data.max_frames
    ...
    def _make_batch_data(self, metrics: dict[str, Any]) -> DataProto:
        ...
        while True:
            ...
            gen_batch = new_batch.pop(
                batch_keys=["input_ids", "attention_mask", "position_ids"],
                non_tensor_batch_keys=["raw_prompt_ids", "multi_modal_data"],
                meta_info_keys=["min_pixels", "max_pixels", "video_fps", "max_frames"],
            )
            ...
            with timer("step", timing_raw):
                ...
                with timer("adv", timing_raw):
                    if "token_level_scores" not in batch.batch:
                        ...
                        if "global_caption_rewards" in reward_metrics:
                            batch.batch["global_caption_rewards"] = reward_metrics["global_caption_rewards"]
                        # if "qa_logs" in reward_metrics:
                        #     qa_logs = reward_metrics["qa_logs"]
                        #     if self.global_step % 10 == 0:
                        #         with open(os.path.join(self.config.trainer.save_checkpoint_path, f"qa_logs_step_{self.global_step}.json"), "w") as f:
                        #             json.dump(qa_logs, f, ensure_ascii=False, indent=2)
                        #     del reward_metrics["qa_logs"]
