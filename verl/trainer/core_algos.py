...
@register_adv_estimator(AdvantageEstimator.GRPO_CAPTION)
def compute_grpo_caption_outcome_advantage(
    token_level_rewards: torch.Tensor, response_mask: torch.Tensor, index: torch.Tensor, eps: float = 1e-6, **kwargs
) -> tuple[torch.Tensor, torch.Tensor]:
    """
    Compute advantage for GRPO, operating only on Outcome reward (with only one scalar reward for each response).
    ...
    """
    outcome_scores = compute_grpo_outcome_advantage(token_level_rewards, response_mask, index, eps=eps)

    caption_mask = kwargs['caption_mask']
    caption_level_scores = kwargs['global_caption_rewards']

    id2score = defaultdict(list)
    id2mean, id2std = {}, {}

    bsz = caption_level_scores.shape[0]
    for i in range(bsz):
        id2score[index[i]].append(caption_level_scores[i])
    ...
    for i in range(bsz):
        caption_level_scores[i] = (caption_level_scores[i] - id2mean[index[i]]) / (id2std[index[i]] + eps)

    returns = caption_level_scores.unsqueeze(-1) * caption_mask


    return (outcome_scores[0], returns), (outcome_scores[1], returns)
